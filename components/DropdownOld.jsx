  ...
  const dropown_items = props.items.map((item, index) => {
    if (item.content) {
      ...
      return (
        ...
        >
          <Link className="dropdown-item menu-2" href={item.link}>
            <span>{item.name} </span>{" "}
      ...
      return (
        <li key={index}>
          <Link className="dropdown-item" href={item.destination_uri}>
            <span>{item.name}</span>
          </Link>
        </li>
      );
    ...
    >
      {props.trigger ? (
        <>
          <button
            className={
              "border-0 dropdown-toggle dropdown-toggle-split border" +
              props.className
            }
            type="button"
        ...
        <button
          ...
          aria-expanded="false"
          style={
            props.name.split(" ").length > 1 ? styles.dropdownButton : null
          }
        >
          {props.name}
        </button>
